...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_backButtonTapped) name:kNotificationInviteFriendsSMSSent object:nil];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    barButton = [[UIBarButtonItem alloc] initWithTitle:@"Continue" style:UIBarButtonItemStyleDone target:self action:@selector(done:)];
    barButton.enabled = FALSE;
    self.navigationItem.rightBarButtonItem = barButton;
    
    leftBarButton = [[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStyleDone target:self action:@selector(_backButtonTapped)];
    ...
    ABAddressBookRequestAccessWithCompletion(self.addressBookRef, ^(bool granted, CFErrorRef error) {
        if (granted) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                UIAlertView *warningAlert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Please go to your Device [Settings] > [Privacy] > [Contacts] and check the settings." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
                [warningAlert show];
            });
        }
    });
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Update window title
    self.title = [NSString stringWithFormat:@"Invite friends (%lu)", (unsigned long)self.selectedContacts.count];
...
- (void)done:(id)sender
{
    ...
    for (int i = 0; i < self.selectedContacts.count; i++) {
        THContact *contact = self.selectedContacts[i];
        [recipents addObject:contact.phone];
    }
